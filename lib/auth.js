...
// authenticator-app invult, wordt 2FA daadwerkelijk actief.
export async function confirm2FASetup(code) {
  const account = await getHousehold();
  if (!account?.twoFactorSecretPending) return false;
  ...
  await saveHousehold({ ...rest, twoFactorSecret: twoFactorSecretPending });
  return true;
}
...
export async function disable2FA() {
  const account = await getHousehold();
  if (!account) return;
  const { twoFactorSecret, twoFactorSecretPending, ...rest } = account;
