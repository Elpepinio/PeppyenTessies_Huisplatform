...
export async function logAiGebruik({ bron = "onbekend", inputTokens = 0, outputTokens = 0, gebruiker = null }) {
  try {
    const kostenUsd = (inputTokens / 1_000_000) * PRIJS_PER_1M_INPUT + (outputTokens / 1_000_000) * PRIJS_PER_1M_OUTPUT;
    const huidig = await redis.get(LOG_KEY);
    const lijst = huidig ? (typeof huidig === "string" ? JSON.parse(huidig) : huidig) : [];
    lijst.push({ datum: new Date().toISOString(), bron, inputTokens, outputTokens, kostenUsd, gebruiker });
